...
Task: Solve the LeetCode problem "faulty-sensor":
...
- Function name should be: maximizeXor
...
**Summary:** Linear scan to first mismatch, then check which sensor's remaining elements align with the other (shifted by one). If both or neither align, return `-1`. O(n) time, O(1) space. Note: the task specifies function name `maximizeXor` which is a copy-paste error — the correct LeetCode name is `badSensor`.
