...
class TestCanBeTypedWords(unittest.TestCase):
    ...
    def test_single_word_typeable(self):
        self.assertEqual(self.s.canBeTypedWords("hello", "z"), 1)

    def test_single_word_not_typeable(self):
        self.assertEqual(self.s.canBeTypedWords("hello", "h"), 0)

    def test_single_char_words(self):
        self.assertEqual(self.s.canBeTypedWords("a b c d", "b"), 3)

    def test_all_words_typeable(self):
        self.assertEqual(self.s.canBeTypedWords("abc def ghi", "z"), 3)

    def test_no_words_typeable(self):
        self.assertEqual(self.s.canBeTypedWords("abc def", "abcdef"), 0)

    def test_all_26_broken(self):
        self.assertEqual(self.s.canBeTypedWords("test", "abcdefghijklmnopqrstuvwxyz"), 0)
