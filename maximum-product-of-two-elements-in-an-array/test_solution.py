...
from solution import minSetSize
...
class TestMinSetSize(unittest.TestCase):
    ...
    def test_min_length(self):
        self.assertEqual(minSetSize([1, 1]), 0)

    def test_all_ones(self):
        self.assertEqual(minSetSize([1, 1, 1, 1]), 0)

    def test_identical_values(self):
        self.assertEqual(minSetSize([5, 5, 5]), 16)

    def test_max_boundary(self):
        self.assertEqual(minSetSize([1000, 1000]), 998001)

    def test_descending_order(self):
        self.assertEqual(minSetSize([5, 4, 3, 2, 1]), 12)

    def test_largest_at_end(self):
        self.assertEqual(minSetSize([1, 2, 3, 10]), 18)

    def test_large_array(self):
        nums = list(range(1, 501))
        self.assertEqual(minSetSize(nums), 499 * 498)
